...
const NavSec = () => {
  ...
  const scrollToSection = (id:any) => {
    ...
    if (isHomePage) {
      ...
      // If we're on another page (like /registration), navigate to home page with hash
      router.push(`/#${id}`);
    }
  ...
  return (
    <div>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-3/4 bg-white/30 backdrop-blur-md shadow-lg transform transition-transform duration-300 ease-in-out ${menuOpen ? "translate-x-0" : "translate-x-full"} z-40 md:hidden p-6 flex flex-col space-y-4`}
      >
        {navLinks.map((link) =>
